...
namespace Stocks.Web.Controllers
{
    ...
    public class TradeController : Controller
    {
        ...
        [Route("[action]/{stockSymbol}/{stockPrice?}")]
        [Route("~/[controller]/{stockSymbol}/{stockPrice?}")]
        public async Task<IActionResult> Index(string stockSymbol = "", int stockPrice = 0)
        {
            _logger.LogInformation($"TradeController.Index()");
            _logger.LogDebug($"TradeController Index stockSymbol: {stockSymbol}");
            _logger.LogDebug($"TradeController Index stockPrice: {stockPrice}");
